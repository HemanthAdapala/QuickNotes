...
class BackupSerializer {
  ...
  static String denormalizeAttachmentUri(String input, String targetDir) {
    ...
    final filename =
        input.substring(BackupFormat.attachmentSchemePrefix.length);
  ...
  static Note deserializeNote(Map<String, dynamic> map) {
    ...
    final List<String> tags =
        rawTags is List ? rawTags.map((e) => e.toString()).toList() : [];
  ...
  static TaskItem deserializeTask(Map<String, dynamic> map) {
    ...
    return TaskItem(
      ...
      status: TaskStatusExtension.fromDbString(
          map['status'] as String? ?? 'waiting'),
      createdAt: created,
      updatedAt: map['updatedAt'] != null
          ? DateTime.parse(map['updatedAt'] as String)
          : created,
      completedAt: map['completedAt'] != null
          ...
          : null,
      reminderEnabled:
          map['reminderEnabled'] == true || map['reminderEnabled'] == 1,
      reminderMode: ReminderModeExtension.fromDbString(
          map['reminderMode'] as String? ?? 'off'),
      reminderTime: map['reminderTime'] != null
          ? DateTime.parse(map['reminderTime'] as String)
          : null,
      notificationId: 0, // Reset to 0 default on restore
      repeatRule: RepeatRuleExtension.fromDbString(
          map['repeatRule'] as String? ?? 'none'),
